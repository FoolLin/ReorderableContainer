...
func _ready():
	if get_parent() is ScrollContainer:
		_scroll_container = get_parent()
		if _scroll_container.has_method("handle_overdrag"):
...
func _handle_input(delta): 
	if _scroll_container != null and _is_press and not _is_hold:
		var scroll_point = _scroll_container.scroll_vertical if is_vertical else _scroll_container.scroll_vertical
...
func _on_start_dragging():
	...
	if _is_smooth_scroll:
		_scroll_container.process_mode = Node.PROCESS_MODE_DISABLED	
...
func _on_stop_dragging():
	...
	if _is_smooth_scroll:
		_scroll_container.pos = Vector2(_scroll_container.scroll_horizontal, -_scroll_container.scroll_vertical)
		_scroll_container.process_mode = Node.PROCESS_MODE_INHERIT	
...
func _handle_auto_scroll(delta):
	...
	var scroll_g_rect = _scroll_container.get_global_rect()
	if is_vertical:
		...
		if upper > mouse_g_pos.y:
			...
			_scroll_container.scroll_vertical -= delta * float(auto_scroll_speed) * 150.0 * factor
		elif lower < mouse_g_pos.y:
			var factor = (mouse_g_pos.y - lower) / (scroll_g_rect.end.y - lower)
			_scroll_container.scroll_vertical += delta * float(auto_scroll_speed) * 150.0 * factor
		else:
			_scroll_container.scroll_vertical = _scroll_container.scroll_vertical
	else:
		...
		if left > mouse_g_pos.x:
			...
			_scroll_container.scroll_horizontal -= delta * float(auto_scroll_speed) * 150.0 * factor
		elif right < mouse_g_pos.x:
			var factor = (mouse_g_pos.x - right) / (scroll_g_rect.end.x - right)
			_scroll_container.scroll_horizontal += delta * float(auto_scroll_speed) * 150.0 * factor
		else:
			_scroll_container.scroll_horizontal = _scroll_container.scroll_horizontal		
